...
```{r}
sigma <- influence(mod_base)$sigma
sigma_points <- sigma[sigma < 2.26] %>% names() %>% as.numeric()
cooksd <- cooks.distance(mod_base)
cooksd_points <- cooksd[cooksd > 0.5] %>% names() %>% as.numeric()
outlier_points <- union(sigma_points, cooksd_points)
outliers <- all$label[outlier_points]
```

Figure \@ref(fig:base_diagnostic)) shows residuals (`resid`), leave-one-out sigma ($\sigma_{[-i]}$) and Cook's distance ($D_i$) for each explanatory variable. In all of these diagnostic plots, there seems to be three or four outliers that have significant influence on the model's fit. The outlier observations with $\sigma_{[-i]}$ less than 2.26 or $D_i$ greater than 0.5 are districts `r outliers`. 


```{r base_diagnostic, results = "asis", warning=FALSE, message = FALSE}
ggnostic(mod_base, columnsY = c(".resid", ".sigma", ".cooksd")) 
```

```{r}
df <- mod_df %>% 
    filter(label %in% outliers) %>%
...
Since the base model does not accou t for the excess zeros in the sample, a better performing model should take into account that districts that have at least one outcome count may be qualitatively different from those that did not have zero count. The model I choose is zero-inflated Poisson, aka mixed Poisson regression. This model assumes that 
    $$\rm{E}[\text{count}_i = k] = P(\text{count}_i = 0) * 0 + P(\text{count}_i > 0) * \rm{E}[\text{count}_i = k \, | \, \text{count}_i > 0],$$
i.e. the count data is model conditional on the logit model predicting $P(\text{count}_i = 0)$. I will use the same set of predictors as the base model regression equation. 

```{r, results = "asis"}
library(pscl)
mod_inflated <- zeroinfl(count ~ key_rep + key_sen + key_gov + 
               women_rep + women_sen + women_gov +  
               medianHouseInc_log + totalPop_log +
               blackPop_log + age18to24_log,
               data = mod_df)
...
The result for the count outcome is very similar to the base model result. In both models, the number of leading women candidates in the Senate and governor races is significantly associated with higher count values. Unlike the base model though, the share of blacks in the population (`blackPop_log`) is not significant when predicting count values. Even when it comes to the logistic compoenent of the model, `blackPopShare_log` has negative coefficient. 
...
The anticipation of the 2018 election was that the Democrats would take at least the House majority while the forecast for the Democrats taking the Senate was unpropitious. The outliers in the previous 
